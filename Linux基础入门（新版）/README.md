...
文件描述符
---------
形式上是一个非负整数，实际上是一个索引值，指向内核为每一个进程所维护的该进程打开文件的记录表。当程序打开一个现有文件或者创建一个新文件时，内核向进程返回一个文件描述符。在程序设计中，一些涉及底层的程序编写往往会围绕着文件描述符展开。这一概念只适用于UNIX、Linux这样的操作系统。

标准错误重定向
-------------
> cat hello\_world.c test.c > result\_cat (错误信息输出在屏幕上)
>
> cat hello\_world.c test.c > result\_cat 2>&1 (错误信息输出在文件中)
>
> 将标准错误重定向到标准输出，再将标准输出重定向到文件，注意把重定向到文件写在前面
>
> 应该在输出重定向文件描述符前加上&，否则shell会当做重定向到另一个文件名为1的文件
>
> cat hello\_world.c test.c &> somefile

tee
---
> echo ‘hello world’ | tee hello
>
> 除了在屏幕输出外，还重定向到了hello文件中

永久重定向
---------
> exec 1>somefile
>
> 使用exec替换当前进程的重定向，将标准输出重定向到一个文件，后面执行的命令的输出都将重定向到文件中，直到退出当前shell，或取消exec的重定向
>
> exec的作用：使用指定的命令替换当前的shell，及使用一个进程替换当前进程，或指定新的重定向

创建输出文件描述符
------------------
默认shell中有9个打开的文件描述符

> cd /dev/fd/; ls -Al (查看当前shell进程中打开的文件描述符)
>
> **使用exec创建新的文件描述符**
>
>exec 3>somefile
...
完全屏蔽命令的输出
-----------------
/dev/null,或称空设备，通常用于丢弃不需要的输出流，或作为用于输入流的空文件，通常由重定向完成。读取它会立即得到一个EOF

> cat hello\_world.c nefile 1>/dev/null 2>&1
...
>
> 将/etc/passwd文件按：分割取第一个字段排序后，使用echo生成一个列表

<hr>

Regular Expression
------------------
代码中简写为regex、regexp或RE，使用单个字符串来描述、匹配一系列符合某个句法规则的字符串。
...
> () 圆括号哦用来定义模式字符串的范围和优先级

- 语法

    - \\: 将下一个字符标记为一个特殊字符、原义字符
    - ^ : 匹配输入字符串的开始位置
    - $ : 匹配输入字符串的结束位置
    - {n}:n是一个非负整数，匹配确定的n次。如o{2}匹配连续的两个o
    - {n,m}:m和n均为非负整数，其中n<=m，最少匹配n次且最多匹配m次。！！！在逗号和两个数之间不能有空格
    - {n,}:n是非负整数，至少匹配n次
    - ？: 当该字符紧跟在任何一个其他限制符（上述诸符号）后时，匹配模式是非贪婪的，即尽可能少地匹配所搜索的字符串。(默认贪婪模式). 如o+?匹配单个o
    - . : 匹配除\n之外的任何单个字符
...
> - [几种正则表达式引擎的语法差异](http://www.greenend.org.uk/rjk/tech/regexp.htmltop)
> - [各语言各平台对正则表达式的支持](http://en.wikipedia.org/wiki/Comparison_of_regular_expression_engines)

regex思维导图

![image](/Linux%E5%9F%BA%E7%A1%80%E5%85%A5%E9%97%A8%EF%BC%88%E6%96%B0%E7%89%88%EF%BC%89/5.png)
